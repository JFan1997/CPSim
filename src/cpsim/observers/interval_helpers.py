...
def __add__(list_of_intervals, another_list_of_intervals):
    assert len(list_of_intervals) == len(
        another_list_of_intervals), "You can't add 2 lists (of intervals) with different number of elements"
    new_list = []
...
def RungeKutta_Integration(dynamics, x_interval, u_interval, step_size):
    ...
    x_next = __add__(x, __sum__(
        [__mul__(k_1, step_size / 6.0), __mul__(k_2, 2.0 * step_size / 6.0), __mul__(k_3, 2.0 * step_size / 6.0),
         __mul__(k_4, step_size / 6.0)]))
...
def reach(dynamics, x0_interval, Us_saved, Xs_saved, unsafeSensors_oneHot, step_size):
    ...
    for i in range(total_steps):
        if Xs_saved:
            x_interval = replace_safe_state_intervals_with_saved_value_intervals(x_interval, Xs_saved_interval[i],
                                                                                 unsafeSensors_oneHot)
