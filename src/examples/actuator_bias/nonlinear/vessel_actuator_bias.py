...
if __name__ == "__main__":
    ...
    # actuator bias attack set(applied to battery power and rudder angle respectively)
    
    # bias_magnitude = np.array([5, np.deg2rad(5)])  
    bias_magnitude = np.array([0, np.deg2rad(5)])  
    # bias_magnitude = np.array([5, 0])  
    # bias_magnitude = np.array([0, 0])  
    ...
    x0_arrr = [x[0] for x in sim.outputs[:max_index + 1]]
    x1_arr = [x[1] for x in sim.outputs[:max_index + 1]]
    psi_arr = [x[2] for x in sim.outputs[:max_index + 1]]
    u_arr = [x[3] for x in sim.outputs[:max_index + 1]]

    u_rudder_arr = [x[1] for x in sim.inputs[:max_index + 1]]
    u_battery_arr = [x[0] for x in sim.inputs[:max_index + 1]]

    ref2 = [x[2] for x in sim.refs[:max_index + 1]]
    ref1 = [x[3] for x in sim.refs[:max_index + 1]]
    ...
    axes[0].plot(t_arr, u_arr, label='Speed')
    axes[0].plot(t_arr, ref1, color='orange', linestyle='--', label='Reference Speed')
    axes[0].scatter(t_arr[bias_start], u_arr[bias_start], color='red', label='Bias Start', zorder=5)
    axes[0].scatter(t_arr[bias_end], u_arr[bias_end], color='g', label='Bias End', zorder=5)
    axes[0].set_ylabel('Speed')
    axes[0].legend()
    axes[0].grid()
    
    axes[1].set_title('Yaw Angle (psi)')
    axes[1].plot(t_arr, np.rad2deg(psi_arr), label='Yaw Angle (degrees)')
    axes[1].plot(t_arr, np.rad2deg(ref2), color='orange', linestyle='--', label='Reference Yaw Angle')
    axes[1].scatter(t_arr[bias_start], np.rad2deg(psi_arr[bias_start]), color='red', label='Bias Start', zorder=5)
    axes[1].scatter(t_arr[bias_end], np.rad2deg(psi_arr[bias_end]), color='g', label='Bias End', zorder=5)
    axes[1].set_ylabel('Angle (deg)')
    axes[1].legend()
    axes[1].grid()

    axes[2].set_title('East')
    axes[2].plot(t_arr, x1_arr, label='East')
    axes[2].scatter(t_arr[bias_start], x1_arr[bias_start], color='red', label='Bias Start', zorder=5)
    axes[2].scatter(t_arr[bias_end], x1_arr[bias_end], color='g', label='Bias End', zorder=5)
    # axes[2].set_ylabel('Position (m)')
    axes[2].legend()
    axes[2].grid()

    axes[3].set_title('Position')
    axes[3].plot(t_arr, x0_arrr, label='Position')
    axes[3].scatter(t_arr[bias_start], x0_arrr[bias_start], color='red', label='Bias Start', zorder=5)
    axes[3].scatter(t_arr[bias_end], x0_arrr[bias_end], color='g', label='Bias End', zorder=5)
    # axes[3].set_ylabel('Position (m)')
    axes[3].legend()
    axes[3].grid()

    axes[4].set_title('Rudder Angle Control Input (u_rudder)')
    axes[4].plot(t_arr, np.rad2deg(u_rudder_arr), label='Rudder Angle Input (degrees)')
    axes[4].scatter(t_arr[bias_start], np.rad2deg(u_rudder_arr[bias_start]), color='red', label='Bias Start', zorder=5)
    axes[4].scatter(t_arr[bias_end], np.rad2deg(u_rudder_arr[bias_end]), color='g', label='Bias End', zorder=5)
    axes[4].set_ylabel('Angle (deg)')
    axes[4].legend()
    axes[4].grid()

    axes[5].set_title('Battery Power Control Input (u_battery)')
    axes[5].plot(t_arr, u_battery_arr, label='Battery Power Input')
    axes[5].scatter(t_arr[bias_start], u_battery_arr[bias_start], color='red', label='Bias Start', zorder=5)
    axes[5].scatter(t_arr[bias_end], u_battery_arr[bias_end], color='g', label='Bias End', zorder=5)
    axes[5].set_ylabel('Power Input')
    ...
    plt.savefig(os.path.join(results_dir, 'vessel_combined_bias_attack.png'))
    ...
    # ref1 = [x[3] for x in sim.refs[:max_index + 1]]
    # y1_arr = [x[3] for x in sim.outputs[:max_index + 1]]
    # ax1.set_title('Speed')
    ...
    # y3_arr = [x[1] for x in sim.outputs[:max_index + 1]]
    # ax3.set_title('East')
    # ax3.plot(t_arr, y3_arr)
    # y4_arr = [x[0] for x in sim.outputs[:max_index + 1]]
    # ax4.set_title('Position')
    # ax4.plot(y3_arr, y4_arr)
    # y5_arr = [x[1] for x in sim.inputs[:max_index + 1]]
    # ax5.set_title('Inputs')
    # ax5.plot(t_arr, y5_arr)
    # y6_arr = [x[0] for x in sim.inputs[:max_index + 1]]
