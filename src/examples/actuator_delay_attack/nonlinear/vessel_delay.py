...
def vessel(x, u, use_imath=False):
    ...
    # Npsi = 0
    # Helper variables
    a1 = Iz * Yv_dot - Nr_dot * Yv_dot + Nv_dot * Yr_dot - Iz * m + Nr_dot * m + (
            Xg * m) ** 2 - Nv_dot * Xg * m - Xg * Yr_dot * m
...
def propulsion(torque, shaft_speed, use_imath=False):
    ...
    rho = 1000
    this_shaft = (1 / Is) * (
            torque - copysign_custom(1, shaft_speed, use_imath) * Kq * rho * D ** 5 * (shaft_speed / (2 * pi)) ** 2)
...
if __name__ == "__main__":
    ...
    #     sim.update_current_ref(ref[i])
    #     # attack here
    #     sim.cur_feedback = bias_attack.launch(sim.cur_feedback, sim.cur_index, sim.states)
    #     sim.evolve()
